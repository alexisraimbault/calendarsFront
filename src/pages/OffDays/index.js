import React, {
  ...
  class OffDays extends Component {
    ...
    componentDidMount() {
      ...
      const {
          sessionToken, history, userInfos, requestGetOffDays,fetchUsers
      } = this.props;
    ...
    createOrDeleteOffDay = (offDay, day) => () => {
      const {
          sessionToken, history, userInfos, requestGetOffDays, addOffDay, deleteOffDay 
      } = this.props;
  
      const isOffDay = !_.isEmpty(offDay);
      if(isOffDay) {
        deleteOffDay(offDay.id,  sessionToken).then(() => {
          requestGetOffDays( _.get(userInfos, 'id'), sessionToken);
        })
      }else{
        const formattedDate = `${moment(day).year()}-${moment(day).month() + 1}-${moment(day).date()}`;
        addOffDay(_.get(userInfos, 'id'), formattedDate, sessionToken).then(() => {
          requestGetOffDays( _.get(userInfos, 'id'), sessionToken);
        })
      }
    }
    ...
    render() {
      ...
      const { userInfos, offDays } = this.props;
          ...
          <div className="calendar-center-container months-container">
            {_.map(daysOfMonth, (dayObject, index) => {
                ...
                <DayCalendarOffDisplay
                    ...
                    day={dayObject}
                    index={index}
                    offDays={_.find(offDays, offDay => moment(offDay.day).isSame(dayObject, 'date'))}
                    createOrDeleteOffDay={this.createOrDeleteOffDay}
                />
            );})}
          </div>
          {isPopupDisplayed && (
  ...
  const mapDispatchToProps = (dispatch) => bindActionCreators({
    requestGetOffDays, addOffDay, deleteOffDay, 
    fetchUsers,
    logout,
  }, dispatch);
