...
import { fetchOperations, fetchMyOperations, createOperation, postDeleteOperation } from '../../../redux/actions/operationActions';
...
class OperationsPopup extends Component {
  ...
  render() {
    ...
    const { operations, towardsCreateOperationPopup, towardsEditOperationPopup, userInfos, isLoading } = this.props;
        ...
        >
          {_.map(operations, operation => (
            <div className="operation-container">
              ...
              {!_.isNil(operation.documents) && !_.isEmpty(operation.documents) && (
                <>
                  <div className="operation-data">{'Documents'}</div>
                  <div className="operation-data">{_.get(operation, 'documents')}</div>
                </>
              )}
              <div className="link-exports">
                <div className="link-export">
                  <div  onClick={_.partial(this.copyToClipBoard, `input-${operation.id}`)}>{'copier lien export : '}</div><input type="text" value={`http://orchestraconseil.fr/calendarexport/${operation.id}`} id={`input-${operation.id}`} />
                  </div>

                <div className="link-export">
                  <div  onClick={_.partial(this.copyToClipBoard, `rdv-${operation.id}`)}>{'copier lien rdv : '}</div><input type="text" value={`http://orchestraconseil.fr/rdv/${operation.id}`} id={`rdv-${operation.id}`} />
                </div>
              </div>
            </div>
          ))}
