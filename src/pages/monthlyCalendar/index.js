...
class MonthlyCalendar extends Component {
  ...
  goToDaily = (defaultDate) => () => {
    ...
    history.push(
      `/calendarday/${moment(defaultDate).dayOfYear()}/${moment(
        defaultDate
      ).year()}`
    );
  };
  ...
  openEditOperationPopup =
    (id, name, data, location, documents = "") =>
    () => {
  ...
  openRdvSettingsPopup = () => {
    const RdvSettingsPopupContent = (
      <RdvSettingsPopup
        closePopup={() => {
          this.setPopupState(false);
        }}
        towardsRdvSettingsPage={this.towardsRdvSettingsPage}
      />
    );
    this.setState({
  ...
  calculateRecession = (operationId) => {
    ...
    const filteredEvents = _.filter(
      this.props.events,
      (event) =>
        moment(event.date).month() + 1 === month &&
        moment(event.date).year() === year &&
        event.type === "amo" &&
    ...
    _.each(filteredEvents, (event) => {
      _.each(event.invitations, (invitation) => {
        const isCa =
          _.get(
            _.split(_.get(invitation, "hours", "all-day/nca"), "/"),
            "[1]",
            "nca"
          ) === "ca";
        const isFullDay =
      ...
      <div className="totals-container">
        {_.map(_.orderBy(operations, "id"), (operation) => {
          const totals = JSON.parse(
            _.get(operation, "totals", "fetchDate: {total1: 0, total2: 0,}")
          );
  ...
  render() {
    ...
    const groupedEvents = _.groupBy(operationsFilteredEvents, (event) =>
      getDayOfYear(moment(event.date)._d)
    );
      ...
      !_.isEmpty(fetchDate) && (
        <div className="calendar-container">
          ...
          <div className="week-navigation-container-month">
            <div className="week-navigation-holder-month">
              <div
                ...
                onClick={this.navigateToPreviousMonth}
              >
              ...
              <div className="top">
                <div onClick={this.openDatePicker}>{`${_.capitalize(
                  moment(new Date(year, month - 1, 1)).format("MMM")
                )}`}</div>
              ...
              <div
                className="calendar-navigation-container"
                onClick={this.navigateToNextMonth}
              >
                <svg viewBox="0 0 512 512" width="20" height="20">
                  <path
          ...
          <div className="switch-action-btn-container">
            <ActionButton
              clickAction={this.towardsDailyCalendar}
              ...
              <div className="week-of-month">
                {_.map(daysOfMonthGroup, (dayObject, index) => {
                  const isOff = _.some(
                    offDays,
                    (offDay) =>
                      moment(offDay.day).format("DD/MM/YYYY") ===
                      moment(dayObject).format("DD/MM/YYYY")
                  );
                  return (
                    //   <CalendarGridColumn
