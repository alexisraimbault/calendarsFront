  ...
  switch (action.type) {
    ...
    case actions.GET_OPERATIONS_SUCCESS:
      ...
      _.each(_.orderBy(operationList, "id"), (operation, index) => {
        ...
        const escapedOperationDocs = _.isNil(operation.documents)
          ? ""
          : operation.documents.replace(regex2, "\n");
    ...
    case actions.GET_OPERATION_SETTINGS_SUCCESS:
      return {
        ...state,
        settings: JSON.parse(action.payload.rdvs_settings),
        loading: false,
        hasErrors: false,
      };

    case actions.DELETE_OPERATION_SUCCESS:
      newEventsPostDelete = _.remove(
        state.operations,
        (item) => item.id !== action.payload
      );
      return {
        ...state,
        operations: newEventsPostDelete,
        loading: false,
        hasErrors: false,
      };
