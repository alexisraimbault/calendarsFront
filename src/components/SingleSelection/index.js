    ...
    const { keyValues, showLabels, name } = this.props;
      ...
      <div className="single-selection">
        <div className="labels">
          {showLabels && _.map(keyValues, keyValue => <div className="value-label">{keyValue.value}</div>)}
        </div>
        <div className="boxes">
          <div className="name">{name}</div>
          {_.map(keyValues, keyValue => {
            const isToggled =  _.isEqual(toggledKey, keyValue.key);
          ...
          return (
            <div className="checkbox" onClick={_.partial(this.setToggle, keyValue.key)}>
